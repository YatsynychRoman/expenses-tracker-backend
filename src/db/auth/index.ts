...
export const getUserByIdAndRefreshToken = async ({ userId, refreshToken }: { userId: number, refreshToken: string }): Promise<User | null> => {
  try {
    const result = await client.queryObject<User>('SELECT id, username, email FROM users WHERE id = $1 AND refresh_token = $2', [userId, refreshToken]);
    if (!result.rows.length) {
      return null;
    }
    return result.rows[0];
...
export const getUserById = async (userId: string): Promise<User | null> => {
  try { 
    const result = await client.queryObject<User>('SELECT id, username, email FROM users WHERE id = $1', [userId]);
    if (!result.rows.length) {
      return null;
    }
    return result.rows[0];
